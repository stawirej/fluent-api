...
Higher order function consists of functions that either takes a function as a parameter or returns a function.
Having them in our approach opens a way to implement fluent API. We will no longer rely on methods returning +
"this" as e.g. for builder pattern approach or other non-functional interface implementations for having fluent api from user perspective.
