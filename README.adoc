...
- Confusing when not using Value Objects and leveraging static language features.
    * E.g. which _String_ is what in _with(String s)_?
    * Not an issue when using strongly typed Value Objects.
- Possible personal style preferences issues:
    * Multiple nested functions in declaration.
    * Only general method names in builder (_with(...)_).
    * No explicit _build()_ method.

== Appendix

=== Internal library implementation limitations

- Using declared library functions as lambdas required declaring as a functional interface (interface with one function).
